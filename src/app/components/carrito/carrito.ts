...
import { Carrito } from '../../model/carrrito.model';
import { GuestCarritoService } from '../../services/guest-carrito';
import { GuestCheckoutServices } from '../../services/guest-checkout';
...
private checkoutService: GuestCheckoutServices,
...
error: err => Swal.fire('Error', err?.error?.message  || 'No se puedo actualizar ', 'error')
});
...
this.reload();
  ...
  error:(err:any )=>{
Swal.close();
Swal.fire('Error', err?.error?.message || 'No se puedo procesar el checkout ', 'error');
