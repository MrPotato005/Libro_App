...
export class LibroComponent implements OnInit {
  ...
  mostrarColumnas: string[] = ['detalles', 'id_Libro', 'titulo', 'editorial', 'edicion', 'idioma', 'fechaPublicacion',
  ...
  delete(): void {
    Swal.fire({
      ...
      confirmButtonColor: 'd#33',
      cancelButtonColor: '#3085d6'
    }).then((result) => {
      if (result.isConfirmed) {
        this.libroService.dalete(this.libro.idLibro).subscribe(() => {
          this.findAll();
          this.libro = {} as Libro;
...
editarLibroCancelar(from: NgForm): void{
...
this.libro = { ...libro};
this.editar = true;
this,this.idEditar = libro.idLibro;
...
this.http.post<{ruta: string}>('http://locahost:8080/api/upload-portada',formData).subscribe(res =>{
